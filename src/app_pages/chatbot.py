...
def generate_demo_graphs(user_input):
    ...
    if any(w in text for w in ["data", "trend", "analysis"]):
        ...
        fig_trend = px.line(
            ...
            markers=True,
            title="Monthly Avg Temperature Trend",
        )
...
def generate_bot_response(user_input):
    ...
    if "temperature" in text:
        responses.append("Temperature profile generated.")
    if "salinity" in text:
        responses.append("Salinity profile included.")
    if any(
        w in text
        for w in ["floats", "region", "arabian sea", "bay of bengal", "indian ocean"]
    ):
    ...
    if not responses:
        responses = [
            "Try asking about temperature, salinity, depth, or float locations."
        ]
...
def display_chat_messages(messages):
    for message in messages:
        if message.get("role") == "system":
            continue
        avatar = (
            USER_AVATAR if message.get("role", "assistant") == "user" else BOT_AVATAR
        )
        with st.chat_message(message.get("role", "assistant"), avatar=avatar):
            ...
            if figs := message.get("figures"):
                for i, fig in enumerate(figs):
                    st.plotly_chart(
                        fig, use_container_width=True, key=f"{message['timestamp']}_{i}"
                    )
            if message.get("map"):
                render_float_map(
                    dark_mode=True, terrain=True, key=f"map_{message['timestamp']}"
                )
...
def render_floatchat():
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ Delete Chat History", key="delete_chat"):
            ...
            if "float_data" in st.session_state:
                del st.session_state["float_data"]
            save_chat_history([])
            ...
            st.session_state.messages = [
                {
                    ...
                    "content": CHATBOT_CONFIG.get(
                        "welcome_message", "Hello! Ask me about ARGO float data."
                    ),
    ...
    prefill = (
        st.session_state.pop("prefill_input", "")
        if "prefill_input" in st.session_state
        else ""
    )
    if prefill:
        user_input = st.text_input(
            "Ask me anything about ARGO ocean data...",
            value=prefill,
            key=f"prefill_{datetime.now().timestamp()}",
        )
    else:
